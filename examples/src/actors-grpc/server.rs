...
// -- Actor definitions (same as actors example) --
...
// -- gRPC service definition (same as grpc-proxying example) --
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1. Set up the actor server and register actors
    ...
    // 2. Extract the axum Router with actor routes
    let actor_router = dapr_server.build_router().await;

    // 3. Combine actor routes with gRPC service using tonic Routes
    let greeter_service = GreeterService::default();
    let routes = tonic::service::Routes::from(actor_router)
        .add_service(GreeterServer::new(greeter_service));

    // 4. Serve with tonic's Server (handles both HTTP/1.1 and HTTP/2)
    let addr = "[::]:50051".parse().unwrap();
    println!("Combined actor + gRPC server listening on: {addr}");
